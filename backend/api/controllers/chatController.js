 ...
 *   1. A unique AES-256-GCM room key is generated per dispute chat room.
 *   2. The room key is encrypted with each participant's ed25519 public key
 *      (via X25519 ECDH — Stellar keys are converted to Curve25519).
 *   3. Messages are encrypted client-side with the room key before sending.
 *   4. The backend stores only ciphertext — it never sees plaintext.
 *
 * Routes:
...
/**
 * POST /api/chat/:escrowId/room-key
 * Body: { encryptedKeys: { [address]: base64EncryptedKey } }
 *
 * Called by the first participant to set up the room. Stores one
 * encrypted copy of the room key per participant address.
 * The backend never sees the plaintext room key.
 */
export const distributeRoomKey = async (req, res) => {
  try {
    ...
    const participants = [escrow.clientAddress, escrow.freelancerAddress, escrow.arbiterAddress]
      .filter(Boolean);
    ...
    // Store each participant's encrypted key copy
    const roomId = `dispute:${escrowId}`;
    const ops = Object.entries(encryptedKeys).map(([address, encryptedKey]) =>
...
/**
 * GET /api/chat/:escrowId/room-key
 *
 * Returns the encrypted room key for the authenticated user.
 * Only the user's own encrypted copy is returned.
 */
...
/**
 * POST /api/chat/:escrowId/messages
 * Body: { ciphertext: string, iv: string, tag: string }
 *
 * Stores an encrypted message. The backend only stores ciphertext.
 * Plaintext is never transmitted to or stored by the server.
 */
...
/**
 * GET /api/chat/:escrowId/messages
 *
 * Returns paginated encrypted messages. Clients decrypt locally.
 */
