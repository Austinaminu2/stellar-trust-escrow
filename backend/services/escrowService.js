 ...
 * Escrow Service
 *
 * All multi-table write operations are wrapped in Prisma transactions
 * with deadlock retry via withTransaction().
 *
 * Covered operations:
 *   - fundEscrow       — creates escrow + initial activity log
 *   - releaseMilestone — updates milestone + escrow balance + activity log
 *   - raiseDispute     — updates escrow status + milestone + activity log
 *   - resolveDispute   — distributes funds + updates both parties + logs
 */
...
/**
 * Creates an escrow record and logs the funding action atomically.
 *
 * @param {object} data
 * @returns {Promise<{ escrow: object, log: object }>}
 */
...
/**
 * Marks a milestone as Approved, decrements escrow remaining balance,
 * and logs the release — all in one transaction.
 *
 * @param {object} params
 * @returns {Promise<{ milestone: object, escrow: object }>}
 */
export async function releaseMilestone({ escrowId, milestoneIndex, amount, callerAddress }) {
  return withTransaction(async (tx) => {
    // Lock the escrow row for update to prevent concurrent releases
...
/**
 * Sets escrow status to Disputed, optionally marks a milestone as Disputed,
 * and creates a Dispute record — atomically.
 *
 * @param {object} params
 * @returns {Promise<{ dispute: object, escrow: object }>}
 */
...
/**
 * Resolves a dispute: updates escrow to Completed, records resolution
 * amounts on the Dispute record, and logs the action.
 *
 * @param {object} params
 * @returns {Promise<{ dispute: object, escrow: object }>}
 */
